...
const int growthRateData::EXPTableErratic[100] = {
...
const int growthRateData::EXPTableFast[100] = {
...
const int growthRateData::EXPTableMedium[100] = {
...
const int growthRateData::EXPTableParabolic[100] = {
...
const int growthRateData::EXPTableSlow[100] = {
...
const int growthRateData::EXPTableFluctuating[100] = {
...
const map<string, float> PokemonData::GENDER_RATE = {
	{"AlwaysMale", 0.0f},
	{"FemaleOneEighth", 0.125f},
	{"Female25Percent", 0.25f},
	{"Female50Percent", 0.5f},
	{"Female75Percent", 0.75f},
	{"FemaleSevenEighths", 0.875f},
	{"AlwaysFemale", 1.0f},
	{"Genderless", -1.0f}
};

PokemonData::PokemonData(int _id,
	...
	: id(_id),
	name(_name), internalName(_internalName),
	species(_species), description(_description),
	type1(TypeData::getTypeFromString(_type1)), type2(TypeData::getTypeFromString(_type2)),
	...
	genderRate(_genderRate), growthRate(_growthRate),

	rareness(_rareness), baseHappiness(_baseHappiness), baseEXP(_baseEXP),
	...
	battlerPlayerY(_battlerPlayerY),
	battlerEnemyY(_battlerEnemyY),
	battlerAltitude(_battlerAltitude),

	eggHatchSteps(_eggHatchSteps),
	eggGroup1(_eggGroup1), eggGroup2(_eggGroup2),

	abilities(_abilities), num_abilities(_num_abilities),
	hiddenAbility(_hiddenAbility),

	learnedMoves(_learnedMoves), num_learnedMoves(_num_learnedMoves),
	eggMoves(_eggMoves), num_eggMoves(_num_eggMoves),

	evolutions(_evolutions), num_evolutions(_num_evolutions) {}

float PokemonData::getFemaleRate(string _genderRate)
{
	return GENDER_RATE.at(_genderRate);
}

int PokemonData::getLevelEXP(string _growthRate, int currentLevel)
{

	if (_growthRate == "Erratic") {
		return growthRateData::EXPTableErratic[currentLevel];
	}
	else if (_growthRate == "Fast") {
		return growthRateData::EXPTableFast[currentLevel];
	}
	else if (_growthRate == "Medium") {
		return growthRateData::EXPTableMedium[currentLevel];
	}
	else if (_growthRate == "Parabolic") {
		return growthRateData::EXPTableParabolic[currentLevel];
	}
	else if (_growthRate == "Slow") {
		return growthRateData::EXPTableSlow[currentLevel];
	}
	else if (_growthRate == "Fluctuating") {
		return growthRateData::EXPTableFluctuating[currentLevel];
	}

	return -1;
}

Gender PokemonData::getRandomGender(float _femaleRate)
{
...
Gender PokemonData::getRandomGender(string _genderRate)
{
	return getRandomGender(getFemaleRate(_genderRate));
}


string PokemonData::checkAbility(string ability) const
{

	if (ability == hiddenAbility) { return ability; }
...
string PokemonData::getRandomAbility(bool includeHidden)
{
	
	int count = num_abilities + 
		(includeHidden ? 1 : 0);

	int randomIndex = rand() % count;

	if (includeHidden && randomIndex == count) {
...
MoveData** PokemonData::getBestMovesAtLevel(int level)
{
	MoveData** ret = new MoveData * [4]{ nullptr,nullptr,nullptr,nullptr };

	int index = 0;

	for (int i = num_learnedMoves - 1; i >= 0; i--)
	{
		if (learnedMoves[i].level > level)
		{
			continue;
		}
		else
		{
			ret[index] = MoveDex::Instance()->getMoveData(learnedMoves[i].name);
			index++;
			if (index > 3)
			{
				break;
...
void PokemonData::print() const
{
	...
	cout << "==No. " << id << ": " << name << endl
		<< "[" << TypeData::getStringFromType(type1) << ", ";
	
	cout << TypeData::getStringFromType(type2) << "]" << endl
		<< "The " << species << " Pokemon" << endl << endl
		<< description << endl << endl <<
		height << " m, " << weight << " kg" << endl << endl;

	//STATS
	cout << "==STATS & EV YIELD" << endl << 
		"HP: "  << HP  << "   EV: " << EV_HP << endl <<
		"ATK: " << ATK << "   EV: " << EV_ATK << endl <<
		"DEF: " << DEF << "   EV: " << EV_DEF << endl <<
		"SPA: " << SPA << "   EV: " << EV_SPA << endl <<
		"SPD: " << SPD << "   EV: " << EV_SPD << endl <<
		"SPE: " << SPE << "   EV: " << EV_SPE << endl << endl;

	//EGG SHIT
	cout << "==EGG/BREEDING" << endl << 
		"Egg Groups: " <<
		(eggGroup1 == "" ? "NONE" : "") << eggGroup1 <<
		(eggGroup2 == "" ? "" : " & ") << eggGroup2 << endl <<
		"Egg Hatch Steps: " << eggHatchSteps << endl << endl;

	//ABILITIES
	cout << "==ABILITIES" << endl;

	for (int i = 0; i < num_abilities; i++) {
		cout << "Ability: " << abilities[i] << endl;
	}

	if (hiddenAbility != "") {
		cout << endl << "Hidden Ability: " << hiddenAbility << endl;
	}
	...
	for (int i = 0; i < num_learnedMoves; i++) {
		cout << "Move: " << learnedMoves[i].name <<  " (learned at level " << learnedMoves[i].level << ")" << endl;
	}
	...
	if (num_evolutions > 0) {
		...
		for (int i = 0; i < num_evolutions; i++) {
			...
			cout << "Evolution: " << evolutions[i].pokemon;

			if(evolutions[i].type == "Level") {
				cout << " at level " << evolutions[i].requirement;
			}
			else if (evolutions[i].type == "Item") {
				cout << " with item " << evolutions[i].requirement;
			}
			else if (evolutions[i].type == "Happiness") {
				cout << " at max happiness";
			}
			else if (evolutions[i].type == "HappinessDay") {
				cout << " at max happiness in the daytime";
			}
			else if (evolutions[i].type == "HappinessNight") {
				cout << " at max happiness in the nighttime";
			}
			else if (evolutions[i].type == "HappinessNight") {
				cout << " at max happiness in the nighttime";
			}
